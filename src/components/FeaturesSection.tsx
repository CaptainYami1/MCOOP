...
const FeaturesSection = () => {
  ...
  const features = [
    {
      title: "Download and join a circle",
      description:
        "Enter your Circle’s group code or create a new circle with 10+ members",
      icon: <IdCard className="w-6 h-6 text-emerald-100" />,
    },
    {
    ...
    },
    {
      ...
      description:
        "Create a secure password to protect your account and keep your circle activities safe",
      icon: <Smartphone className="w-6 h-6 text-emerald-100" />,
    },
    {
      title: "Choose your Plan",
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {features.map((feature, index) => (
            <div
              key={index}
              className="bg-[#EAEAEA1A] border border-[#EAEAEA33] rounded-3xl p-8 hover:bg-[#EAEAEA]/20 transition-colors"
            >
              <div className="w-12 h-12 bg-emerald-800/50 rounded-xl flex items-center justify-center mb-6 border border-[#E5E5E5]/80">
                {feature.icon}
              </div>
              <h3 className="font-semibold text-base leading-[150%] tracking-normal text-white mb-3">
                {feature.title}
              </h3>
